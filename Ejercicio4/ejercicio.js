...
const programmingBooks = [
  ...
  for (let i = 0; i < programmingBooks.length; i++) {
    if (programmingBooks[i].includes(tituloIngresado)) {
      found = true;
      break;
    }
  }
  ...
  if (found) {
    alert(`El título "${tituloIngresado}" se encuentra en la lista.`);
  } else {
    alert(`El título "${tituloIngresado}" no se encuentra en la lista.`);
  }
